...
func _physics_process(delta: float) -> void:
	var moveInput: = Vector2(Input.get_action_strength("ui_right") - Input.get_action_strength("ui_left"), Input.get_action_strength("ui_down") - Input.get_action_strength("ui_up"))
	var speed: = 150 if !Input.is_action_pressed("ui_cancel") else 75
	
	match mode:
		1: heartModeRed(delta, speed, moveInput)
		2: heartModeBlue(delta, speed, moveInput)
	
	match round(rad2deg($Sprite.rotation)):
...
func changeMode(setMode: = 0, setDirection: = 90.0) -> void:
	match setMode:
		...
		3: $Sprite.modulate = Color(0, 0.75, 0)
		4: $Sprite.modulate = Color(1, 1, 0)
	$Sprite.rotation = deg2rad(setDirection)
...
func heartModeBlue(delta, speed, moveInput):
	...
	var heartAngle: = round(rad2deg($Sprite.rotation))
	...
	if !is_on_floor():
		floor_snap = false
		if slammed: motion_velocity = Vector2(cos($Sprite.rotation), sin($Sprite.rotation)) * maxFallSpeed
		else: motion_velocity += Vector2(cos($Sprite.rotation), sin($Sprite.rotation)) * gravity * delta
	...
	if is_on_floor():
		...
		if jumpDirection[heartAngle]:
			floor_snap = false
			motion_velocity -= Vector2(cos($Sprite.rotation), sin($Sprite.rotation)) * 180.0
