...
onready var richTextEffects: Array = [
	preload("res://RPGText/RichTextEffects/Tremble/Tremble.tres"),
]

onready var menuOptionScr: = preload("res://Battle/Hud/MenuOption.gd")
onready var rpgTextScr: = preload("res://RPGText/RPGText.gd")

onready var playerHeart: = $"../HeartLayer/PlayerHeart"
onready var enemy: = $"../Enemy"
onready var target: = preload("res://Battle/HUD/Target/Target.tscn")
...
func _process(_delta: float) -> void:
	if menuNo == 1 and menuCoord[0] == $Buttons/Item.id:
		for n in get_tree().get_nodes_in_group("MenuOption"):
			if n.ID == menuCoord[menuNo] and menuNo != 0:
...
func menuFightEnemy():
	...
	t.connect("playerMiss", enemy, "createDamageWriter", [Vector2(270, 75), 114, 1, 1, 0, 30])
...
func menuActText():
	...
	var actText: Array = enemy.actChoiceText[actSelection][menuCoord[1]]
